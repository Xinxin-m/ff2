...
'''
    TODO: 
        - modify the file in optimization > ff_scenario_time: 
            dataset_scenario = time_const90, chunk_size=100 (if None then it'll use the entire sequence), we want a window of size 100 (i.e. time [a,a=50])
        - check lass RpodDataset(Dataset) > getitem (get a random sample from dataset) and getix (get a specific sample given index)
        - normalize:
