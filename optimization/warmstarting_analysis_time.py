...
def for_computation(input_iterable):
    # extract data from the input, initialize the output dictionary, sample test data, solve an initial convex problem, perform optimization using various warmstart strategies, and save the results.
    ...
    else: # pick one sample from dataset
        if not mdp_constr:
    ...
    try:
        # solve problem with convex relaxation (without obstacles), and use it as warmstart
    ...
    # solve the optimization problem using convex relaxation as starting point
    if np.char.equal(feas_cvx,'optimal'): 
        ...
        # J_vect is the cost for each iteration of the scp optimizer, sometimes it's useful to see how the cost converges
    ...
    if np.char.equal(feas_cvx,'optimal'):
        ...
        # Solve SCP using transformer output as warmstart
    ...
    if np.char.equal(feas_cvx,'optimal') and (not (model_dag is None)):
        ...
        # Solve SCP using model _dag (second model)
...
if __name__ == '__main__':
    ...
    transformer_model_name = 'checkpoint_ff_time_90_random' # check in manage time which transformer class you're using, e.g. AutonomousFreeflyerTransformer_pred_time or without pred_time
    transformer_model_name_dag = None # enter a second model to compare
    import_config = DT_manager.transformer_import_config(transformer_model_name)
    set_start_method('spawn') # allows for parallel processing
    num_processes = 20
    ...
    if not(transformer_model_name_dag is None):
        ...
        model_dag = None  # another model to compare with, can be set to None

    # List of possible time allotted for the trajectory
    ttg_com_list = np.arange(T_min, T_max+1, 20.0)

    for ttg_com in ttg_com_list: # select all final times that are considered (if final time=90, the analysis is performed only once)
        # Parallel for inputs
        ...
        # run the for loop in parallel on the number of processes selected before
        # output of for loop uses for_computation and the result
